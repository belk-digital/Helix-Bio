...
import { getOgImageUrl, toAbsoluteUrl } from '@/lib/utils'
...
export default async function Homepage() {
  ...
  const schema = {
    ...
    '@graph': [
      ...
      UNIFIED_ORGANIZATION_NODE,
      {
        ...
        'itemListElement': validProducts.map((p: any, idx: number) => {
          const rawImg = p.image || p.images?.[0]?.image?.url || ''
          const absoluteImageUrl = toAbsoluteUrl(baseUrl, rawImg)
          const priceValue = (typeof p.price === 'number' ? p.price : parseFloat(p.price || '0')).toFixed(2)
          const descriptionText = p.description || p.shortDescription || p.seoDescription || 'High-purity research peptide.'
          const isAvailable = (typeof p.stock === 'number' ? p.stock : 0) > 0 ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock'
  ...
  return (
    <>
      ...
      <JsonLd id="schema-home" data={schema} />
    </>
  )
}
